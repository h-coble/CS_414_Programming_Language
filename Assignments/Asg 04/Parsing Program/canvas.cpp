...
bool parseExpression(std::stack<char> expression, std::stack<int> state)
{
	...
	if (foundAction == "")
	{
		std::cout << " Undefined action; expression failed because it isn't
			supported.\n";
			return false;
	}
	...
	else
	{
		...
		case 'S':
			...
			std::cout << foundAction << ": Shift " << foundAction << " onto stack,
		...
		case 'R':
			...
			case '6':
				...
				break;
			default:
				std::cerr << "Rule with that number doesn't exist within
					original parse table\n";
					return false;
			}
			tempStr.erase(0, 1);
...
int main() {
	...
	while (!inFile.eof())
	{
		...
		std::cout << std::setw(25) << std::left << "State Stack" << std::setw(25)
			<< std::left << "Input" << "Action\n";
		if (parseExpression(expStack, stateStack))
			std::cout << "\n\nExpression is supported by the rules given by the
			table.\n";
		else
